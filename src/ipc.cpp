    ...
    void init_pipe_server(
        ...
        while (true) {
            ...
            if (hPipe == INVALID_HANDLE_VALUE) {
                LOG_ERROR(
                    "{} -> Error creating a named pipe. Last error: {}", __func__,
            ...
            try {
                ...
                const auto write_success = WriteFile(
                    hPipe,                // handle to pipe
                    response_str.c_str(), // buffer to write from
                    response_str.size(),  // number of bytes to write
                ...
                if (!write_success || response_str.size() != bytes_written) {
                    LOG_ERROR(
                        "{} -> Error writing file. Last error: {}", __func__,
                        koalabox::win_util::get_last_error()
                    );
                    return;
                }
